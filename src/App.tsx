...
export default function App() {
  ...
  // Initialize data from localStorage and Appwrite
  useEffect(() => {
    try {
      ...
      if (savedUser) {
        setUser(safeJsonParse<User>(savedUser, null));
      }
      if (savedAddresses) {
        setAddresses(safeJsonParse<Address[]>(savedAddresses, []));
      }
      if (savedOrders) {
        setOrders(safeJsonParse<Order[]>(savedOrders, []));
      }
        ...
        databases
          ...
          .then((response) => {
            if (response.documents && response.documents.length > 0) {
              const appwriteOrders: Order[] = response.documents.map(
                (o: any) => ({
                  id: o.$id,
              ...
              setOrders((prev) => {
                ...
                const newOrders = appwriteOrders.filter(
        ...
        prev.map((order) => {
          if (
            ...
            if (order.timer > 0) {
              ...
              return { ...order, status: "onWay", timer: 0 };
            }
          }
          return order;
        })
  ...
  const handleOnboardingComplete = (data: {
    ...
    setAddresses([
      { id: addressId, label: t.home, details: data.homeAddress },
    ]);
  };
  ...
  // Show onboarding if user not logged in
  if (!user) {
    return (
      <div
        dir={lang === "ar" ? "rtl" : "ltr"}
      ...
      <div className="relative mx-auto h-screen max-w-md overflow-hidden shadow-2xl shadow-yellow-300/20">
        ...
        {flyingImages.map((img) => (
          <FlyingImage
            ...
            to={img.to || img.from}
            onComplete={() =>
              setFlyingImages((prev) => prev.filter((i) => i.id !== img.id))
            }
          />
        ))}
