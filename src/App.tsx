...
export default function App() {
  ...
  useEffect(() => {
    try {
      ...
      databases.listDATABASE_ID ? null : databases.listDocuments(DATABASE_ID, ORDERS_COLLECTION_ID, [
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setOrders(prev => prev.map(order => {
        if (order.status === "preparing" || order.status === "قيد التحضير" || order.status === "pending") {
          if (order.timer > 0) {
